...
\section*{Intro}
\label{sec:org8efa0e8}
\begin{frame}[label={sec:orga03d177}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:org720655e},fragile]{Unary type constructors}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org7af095f},fragile]{Higher arity}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org394d263},fragile]{Kinds}
...
\begin{frame}[label={sec:orgd9648ee},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:orgfe4dd35},fragile]{Higher-order functions}
...
\begin{frame}[label={sec:orgb42c1c9},fragile]{Higher-order types (1)}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org147f92f},fragile]{Higher-order types (2)}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgbb19d1f}]{Higher-order types (3)}
...
\begin{frame}[label={sec:org0ef8d61}]{The most important question}
\begin{center}
...
\section{Functor}
\label{sec:org5f41e4b}
\begin{frame}[label={sec:org84fab62}]{Function Application}
...
\begin{frame}[label={sec:orgb73220f},fragile]{Function Application}
...
\begin{frame}[label={sec:orgad1ef7f},fragile]{Function Application}
...
\begin{frame}[label={sec:org2ab3d7f},fragile]{Context: optional value}
...
\begin{frame}[label={sec:orge58568c},fragile]{Context: optional value}
 \begin{itemize}
...
\begin{frame}[label={sec:org024a16a},fragile]{Context: optional value}
 \begin{minted}[]{scala}
//  apply        : (A => B) =>        A  =>        B
...
\begin{frame}[label={sec:org36577e0},fragile]{Many contexts}
 \begin{itemize}
...
\begin{frame}[label={sec:org25f3f12},fragile]{Many applies}
 \begin{minted}[]{scala}
def apply           : (A => B) =>        A  =>        B
...
\begin{frame}[label={sec:org123021a}]{Repetition is}
\begin{center}
...
\begin{frame}[label={sec:org38df07c},fragile]{Functoriana}
 \begin{minted}[]{scala}
trait Functor[F[_]] {
...
\begin{frame}[label={sec:orgab64fcb}]{What about the second rule?}
...
\begin{frame}[label={sec:org8f8a9c8}]{There can be only one}
There is only one lawful implementation of Functor for a given type.
\end{frame}

\begin{frame}[label={sec:org52f9597},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org9d3e896},fragile]{Outcome}
 \begin{itemize}
...
\label{sec:orgc18d9b2}

\begin{frame}[label={sec:org74ff72e},fragile]{Next step}
 \begin{minted}[]{scala}
val v1 = Option(42)
val v2 = Option(100)
...
\begin{frame}[label={sec:orgc61e099},fragile]{Next step}
 \begin{minted}[]{scala}
def apply :  (A => B) =>   A  =>   B
...
\begin{frame}[label={sec:org80a72d4},fragile]{Solution}
 \begin{minted}[]{scala}
val v1 = Option(42)
val v2 = Option(100)
...
\begin{frame}[label={sec:orgc1b9c4e},fragile]{Following the same idea}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgfc30ded},fragile]{What's about pure?}
 \begin{columns}
...
\begin{frame}[label={sec:org575b03e},fragile]{Another perspective}
...
\begin{frame}[label={sec:orge42a446},fragile]{From \texttt{Applicative} to \texttt{Monoidal}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:org87f4334},fragile]{From \texttt{Applicative} to \texttt{Monoidal}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:org065fc1b},fragile]{From \texttt{Applicative} to \texttt{Monoidal}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:orgca547fe},fragile]{From \texttt{Monoidal} to \texttt{Applicative}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:org5725efc},fragile]{From \texttt{Monoidal} to \texttt{Applicative}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:orgbd3b72e},fragile]{Monoid part of \texttt{Monoidal}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Monoid[A] {
...
\begin{frame}[label={sec:org8b87a29},fragile]{Applicative laws}
 \begin{enumerate}
...
\begin{frame}[label={sec:org78e55a7},fragile,t]{Parsing}
 \vspace*{20px}
...
\begin{frame}[label={sec:org93ff4f0},fragile,t]{Simple parser}
 \begin{minted}[]{scala}
case class Parser[A](parse: String => ParserResult[A])
\end{minted}
...
\begin{frame}[label={sec:orgc8f745d},fragile]{Abstracting \texttt{char}}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org56d71fa},fragile,t]{Repeating the parser}
 \begin{onlyenv}<1>
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org25f6309},fragile,t]{Repeating the parser}
 \begin{onlyenv}<1>
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org61f63ad},fragile,t]{(invalid) UUID parser}
 \begin{onlyenv}<1>
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org167d1e9},fragile,t]{Space invaders}
 \begin{minted}[]{scala}
def uuid: Parser[String] = /* ... */
...
\begin{frame}[label={sec:orgad1362f}]{So\ldots{}}
\begin{center}
\includegraphics[height=5cm]{images/applicative-confusion.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgea0eedb},fragile,t]{Simple case class example}
 \begin{onlyenv}<1->
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgde63ee0},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orgaf51d91}]{Outcome}
\begin{itemize}
...
\section{Validation}
\label{sec:org7d7e0e2}

\begin{frame}[label={sec:org939b6cb},fragile]{Validation}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgff3dcef},fragile]{Person}
 \begin{minted}[]{scala}
case class Person(name: String, age: Int)
\end{minted}
...
\begin{frame}[label={sec:orgf12f6d6},fragile]{Person}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org3b2dffc},fragile]{Quest for 'all errors'}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org763159d},fragile]{What's wrong with Either?}
...
\begin{frame}[label={sec:org87c911b},fragile]{Doing as much as we can}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org0d88e2b},fragile]{Coding time}
 \begin{itemize}
