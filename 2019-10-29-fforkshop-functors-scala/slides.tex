...
\section*{Intro}
\label{sec:org86a2841}
\begin{frame}[label={sec:org0e9ad55}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:orgba7b6ec}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:orgdeabb0b},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org3b0c7a5}
\begin{frame}[label={sec:org936af91}]{Agenda}
\begin{itemize}
...
\begin{frame}[label={sec:org6059e35},fragile]{Before we start}
 \begin{minted}[]{bash}
...
\section{Type theory}
\label{sec:org824531d}
\begin{frame}[label={sec:org094b307}]{Values and types}
...
\begin{frame}[label={sec:org468bc07},fragile]{Nullary value constructors}
...
\begin{frame}[label={sec:org14734a4},fragile]{Unary value constructors: function}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orge941d20},fragile]{Unary value constructors: class}
...
\begin{frame}[label={sec:orga85fde7},fragile]{Higher arity}
...
\begin{frame}[label={sec:org5c9ee95},fragile]{Nullary type constructors}
...
\begin{frame}[label={sec:org1ae19cf},fragile]{Unary type constructors}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgaf1b042},fragile]{Higher arity}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org4eb5f41},fragile]{Kinds}
...
\begin{frame}[label={sec:orgc51d83b},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:org14fd4be},fragile]{Higher-order functions}
...
\begin{frame}[label={sec:org1370e52},fragile]{Higher-order types}
 \begin{minted}[]{scala}
case class WickedWrapper[F[_]](level: F[Int], desc: String)
case class WickedDoubleWrapper[F[_], A](value: F[A])
\end{minted}

\pause

\texttt{WickedWrapper} takes one type \texttt{F} to produce new type \texttt{WickedWrapper[F[\_]]}.
But \texttt{F} here is not a concrete type, it's also a type constructor.

\pause

\begin{itemize}
\item \texttt{F} has kind \(* \rightarrow *\).
\item \texttt{WickedWrapper} has kind \((* \rightarrow *) \rightarrow *\) and is unary.
\item \texttt{WickedDoubleWrapper} has kind \((* \rightarrow *) \rightarrow * \rightarrow *\)
and is binary.
\item \(\rightarrow\) is right-associative, so \(* \rightarrow * \rightarrow *
  \rightarrow *\) is the same as \(* \rightarrow (* \rightarrow (* \rightarrow
  *))\).
\end{itemize}
...
\begin{frame}[label={sec:org5910307}]{The most important question}
\begin{center}
...
\section{Functor}
\label{sec:org0bd7951}
\begin{frame}[label={sec:orgf2a019f}]{Function Application}
...
\begin{frame}[label={sec:org09c65a0},fragile]{Function Application}
...
\begin{frame}[label={sec:org3978f2c},fragile]{Function Application}
...
\begin{frame}[label={sec:org09e6d81},fragile]{Context: optional value}
...
\begin{frame}[label={sec:orgdafcda1},fragile]{Context: optional value}
 \begin{itemize}
...
\begin{frame}[label={sec:org00db9df},fragile]{Context: optional value}
 \begin{minted}[]{scala}
//  apply        : (A => B) =>        A  =>        B
...
\begin{frame}[label={sec:orgf4390b7},fragile]{Many contexts}
 \begin{itemize}
...
\begin{frame}[label={sec:orgff93ab6},fragile]{Many applies}
 \begin{minted}[]{scala}
def apply           : (A => B) =>        A  =>        B
...
\begin{frame}[label={sec:org1084e9d}]{Repetition is}
\begin{center}
...
\begin{frame}[label={sec:org70d5e77},fragile]{Functoriana}
 \begin{minted}[]{scala}
trait Functor[F[_]] {
...
\begin{frame}[label={sec:orga2d6dc2},fragile]{Code responsibly, know the laws}
 \begin{enumerate}
...
\begin{frame}[label={sec:org7d146e7}]{Sum type: Option}
\begin{center}
...
\begin{frame}[label={sec:org583e2e3},fragile]{Product type: List}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org468bf23},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org57dc9c9},fragile]{Breaking the first rule}
 \begin{minted}[]{scala}
object OptionImpl {
  implicit val optionFunctor: Functor[Option] = {
    def fmap[A, B]: (A => B) => Option[A] => Option[B] =
...
\begin{frame}[label={sec:orgaa25eae}]{Breaking the second rule}
...
\begin{frame}[label={sec:orgc73be4d},fragile]{Outcome}
 \begin{itemize}
...
\label{sec:orgc601eed}

\begin{frame}[label={sec:org2723076},fragile]{Next step}
 \begin{minted}[]{scala}
val v1 = Option(42)
val v2 = Option(100)
...
\begin{frame}[label={sec:org1bd2436},fragile]{Next step}
 \begin{minted}[]{scala}
def apply :  (A => B) =>   A  =>   B
...
\begin{frame}[label={sec:orgf458373},fragile]{Solution}
 \begin{minted}[]{scala}
val v1 = Option(42)
val v2 = Option(100)
...
\begin{frame}[label={sec:org4bcaa0e},fragile]{Following the same idea}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org2971c08},fragile]{What's about pure?}
 \begin{enumerate}
...
\begin{frame}[label={sec:orgf63d7c4},fragile]{Another perspective}
...
\begin{frame}[label={sec:org222fb34},fragile]{From \texttt{Applicative} to \texttt{Monoidal}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:org341d05f},fragile]{From \texttt{Applicative} to \texttt{Monoidal}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:org10a2f11},fragile]{From \texttt{Applicative} to \texttt{Monoidal}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:org981ad03},fragile]{From \texttt{Monoidal} to \texttt{Applicative}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:org27e47cf},fragile]{From \texttt{Monoidal} to \texttt{Applicative}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:org9aa3bf5},fragile]{From \texttt{Monoidal} to \texttt{Applicative}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Applicative[F[_]] extends Functor[F] {
...
\begin{frame}[label={sec:orge7a2ccf},fragile]{From \texttt{Monoidal} to \texttt{Applicative}}
 \begin{minted}[]{scala}
class MonoidalToApplicative[F[_]: Monoidal]() extends Applicative[F] {
  override def pure[A](a: A): F[A] = Monoidal[F].unit.fmap(_ => a)
...
\begin{frame}[label={sec:org799d1fd},fragile]{Monoid part of \texttt{Monoidal}}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Monoid[A] {
...
\begin{frame}[label={sec:org5eaeba1},fragile]{Applicative laws}
 \begin{enumerate}
...
\begin{frame}[label={sec:org440df7b},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org5bf5b1a},fragile]{Outcome}
 \begin{itemize}
...
\section{Parsing}
\label{sec:org12cd8b8}

\begin{frame}[label={sec:orga9108e9},fragile,t]{Parsing}
 \vspace*{20px}
...
\begin{frame}[label={sec:org3f7e016},fragile,t]{Simple parser}
 \vspace*{20px}
...
\begin{frame}[label={sec:orge106506},fragile]{Abstracting \texttt{char}}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgc91b3f4},fragile,t]{Repeating the parser}
 \begin{onlyenv}<1>
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgcbdb0a8},fragile,t]{Repeating the parser}
 \begin{onlyenv}<1>
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org0c6eb64},fragile,t]{(invalid) UUID parser}
 \begin{onlyenv}<1>
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org3c7301c},fragile,t]{Space invaders}
 \begin{minted}[]{scala}
def uuid: Parser[String] = /* ... */
...
\begin{frame}[label={sec:org02a0f2f}]{So\ldots{}}
\begin{center}
\includegraphics[height=5cm]{images/applicative-confusion.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org1ab7a55},fragile,t]{Simple case class example}
 \begin{onlyenv}<1->
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org31b8621},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd2c86b5}]{Outcome}
\begin{itemize}
...
\section{Validation}
\label{sec:org475f27f}

\begin{frame}[label={sec:orge6b509d},fragile]{Validation}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org67361ac},fragile]{Person}
 \begin{minted}[]{scala}
case class Person(name: String, age: Int)
\end{minted}
...
\begin{frame}[label={sec:org85eb2f0},fragile]{Person}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orga0e0f09},fragile]{Quest for 'all errors'}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orga77e165},fragile]{What's wrong with Either?}
...
\begin{frame}[label={sec:org553a68a},fragile]{Doing as much as we can}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgb8dac5b},fragile]{Coding time}
 \begin{itemize}
...
\section*{Final words}
\label{sec:org3628c0f}
\begin{frame}[label={sec:orga91ea51},fragile]{Recap}
 \begin{itemize}
...
\section{Questions?}
\label{sec:org3d07e82}

\section{Thank you very much!}
\label{sec:orgfc19c43}
\end{document}
