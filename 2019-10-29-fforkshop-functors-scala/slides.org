...
~WickedWrapper~ takes one type ~F~ to produce new type ~WickedWrapper[F[_]]~.
But ~F~ here is not a concrete type, it's also a type constructor.

#+BEAMER: \pause

- ~F~ has kind $* \rightarrow *$.
- ~WickedWrapper~ has kind $(* \rightarrow *) \rightarrow *$ and is unary.
- ~WickedDoubleWrapper~ has kind $(* \rightarrow *) \rightarrow * \rightarrow *$
  and is binary.
- $\rightarrow$ is right-associative, so $* \rightarrow * \rightarrow *
...
- <1-> It's easy to write parser combinators, if you
  - don't care about error reporting
