...
\section*{Intro}
\label{sec:org2333c61}
\begin{frame}[label={sec:org0e75163}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:orgc28d835}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:org9e09115},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org8d56de3}
\begin{frame}[label={sec:org2158e90}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\begin{frame}[label={sec:org0e2f0f4},fragile]{Before we start}
 \begin{minted}[]{bash}
...
\label{sec:orgd21818d}

\begin{frame}[label={sec:org71d4e67}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:orgbe4d1a4},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgbcc869e},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgd3afed0},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org3b430e5},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgdca8202},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgf8b8fcc},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:org41a404e},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org80f65a4}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org315822a},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org74a4666},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:orgb4747f8},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org714efa4}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:orgab17acc},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org2cfe2fb},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org056a569},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org2f0436f}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:org27b218f},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:org8aad4d6},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org975c53d}]{What have we done?}
...
\begin{frame}[label={sec:orgea544b1},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:org5f0f26c},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:orgeb16300}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org6e0deaa}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:orgea89174}

\begin{frame}[label={sec:orgad7100d},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgfa9c194}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:orgba9e120}]{Semigroup}
...
\begin{frame}[label={sec:org6492c2c},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:org2f0c966}]{Important!}
...
\begin{frame}[label={sec:org50b99c9},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org1696524},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org90730fe},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:org2dbf5f2}]{You're developer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org8a8c855},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgb61b822},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:orgbc597d8}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org969f962}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgb37ed54},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:orgc1614e8}

\begin{frame}[label={sec:org66974cc}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:orgfd1f667}]{Monoid}
...
\begin{frame}[label={sec:orga436da6}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:org9fe4b82},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orgcbfaf58},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:orgda21c23}]{Commutativity}
\begin{itemize}
...
\begin{frame}[label={sec:orgb763bb2}]{The most important question}
\pause
...
\section{Fibonacci}
\label{sec:orgc8c7018}

\begin{frame}[label={sec:orgd067220}]{The Fibonacci numbers}
\begin{align*}
  F_0 &= 0 \\
  F_1 &= 1 \\
...
\begin{frame}[label={sec:orgb122e67},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org2557036},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgdb3e338},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org62160be},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org80febaa}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:org9e6c2cd},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb1b5a8a}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgf2f28d5}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org2cfe862}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org71135d3},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org0a76082}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org3bf3000}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org505bd4a},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd196763}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:org6d47785},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:org453ff97}

\begin{frame}[label={sec:org59d4a9c}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org11be874}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org3d24b07},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org7945aa6},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org03f4247},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:org0108b93},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org498f80c},fragile]{Homework}
...
\section{Logger}
\label{sec:orga9a90dc}

\begin{frame}[label={sec:org400034d}]{Many monoids}
...
\begin{frame}[label={sec:org2b2d052},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org660db07},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb98746e},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgd3106a1},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org1cc0061},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org5ed9414},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orgcfbc715},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:orgb89d095}
\begin{frame}[label={sec:orge74e214},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section{Questions?}
\label{sec:orgf305e60}

\section{Thank you very much!}
\label{sec:org8975395}
\end{document}
