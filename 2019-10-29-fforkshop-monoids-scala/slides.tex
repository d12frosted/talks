...
\section*{Intro}
\label{sec:org2f97e0c}
\begin{frame}[label={sec:orga239536}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:org8fde4e4},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org3854255},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:org6713f4f},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org600a52a},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:org036b449},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:orgb4f8e49}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org4824c6f},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:orgb3d819d},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:orgc0a6c0f},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:orgd3dedd3}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org81c513b},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org2240e26},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org66d0c58},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org7d48ffa}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:org5b820e8},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orgda07bb2},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org26aad28}]{What have we done?}
...
\begin{frame}[label={sec:orgf7759bf},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:org7f0e46c},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org18f7f3c}]{Type classes: extensibility}
...
\begin{frame}[label={sec:orgd2ab3c1}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:orgb93f246}

\begin{frame}[label={sec:orgf154545},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org1e24e74}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:org4e6114f}]{Semigroup}
...
\begin{frame}[label={sec:orgc830e40},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orge2bd64b}]{Important!}
...
\begin{frame}[label={sec:org8bf6fb7},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org6c16c52},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org5b06229},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orgb371740}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org6a4c920},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org593d7aa},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org353225d}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org1008cdc}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orga44bfe9},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:org1fbd741}

\begin{frame}[label={sec:org4cca67e}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org67f97ff}]{Monoid}
...
\begin{frame}[label={sec:org2ed997b}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:org9653dc2},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\section{Fibonacci}
\label{sec:org9287d4f}

\begin{frame}[label={sec:org524a371}]{The Fibonacci numbers}
\begin{align*}
  F_0 &= 0 \\
  F_1 &= 1 \\
...
\begin{frame}[label={sec:org9588cc3},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgafdf9e8},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org33108e2},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgd2516af},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org7849595}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:org40876c9},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org4f86590}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org96d3435}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org31e5b69}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:orgc9718c8},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org3917968}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org569e975}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:org3f9dc10},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:org8480314}

\begin{frame}[label={sec:org7d5d39b}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org3371508}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org280c0d8},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:orgcee74be},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org3277762},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:orgc1b6853},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org173df2b},fragile]{Homework}
...
\section{Logger}
\label{sec:orgef2858c}

\begin{frame}[label={sec:orgc049924}]{Many monoids}
...
\begin{frame}[label={sec:orge316ab8},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org48d9837},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org7a032a7},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgee37295},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgba5655a},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org83221b8},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orge7d86b4},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:org6d5a346}
\begin{frame}[label={sec:org3bf96a1},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section{Questions?}
\label{sec:org214f1c0}

\section{Thank you very much!}
\label{sec:org4d71097}
\end{document}
