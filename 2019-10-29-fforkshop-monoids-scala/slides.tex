...
\section*{Intro}
\label{sec:orgf5731f4}
\begin{frame}[label={sec:org894460b}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:orgdb11c5f}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:orga6e54ee},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:orge5504b6}
\begin{frame}[label={sec:orgde18142}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\begin{frame}[label={sec:org51aec72},fragile]{Before we start}
 \begin{minted}[]{bash}
...
\label{sec:org482a11c}

\begin{frame}[label={sec:org0c1d214}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:orgf610d67},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org1e623af},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org4603505},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org35f0729},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orga387b3c},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org20dbe16},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:org4912cd8},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org7f3e25c}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org05e2b57},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org8a8d4d9},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org0937a2d},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org5fdbe77}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:orgec46a7f},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org3f5a0dd},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org8768f78},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org197853a}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:org53c7209},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:org4e360c9},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org2cc22c7}]{What have we done?}
...
\begin{frame}[label={sec:orgc2cf154},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:org6f41b91},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:orgf201ae0}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org63043ec}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:org705240a}

\begin{frame}[label={sec:orgdee9e3a},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org4fdbfac}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:org281a95b}]{Semigroup}
...
\begin{frame}[label={sec:org0342b55},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:org92a22b1}]{Important!}
...
\begin{frame}[label={sec:org24420d5},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc52b230},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org43ee5c5},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orgc810bfd}]{You're developer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org25ecec4},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org431a51b},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:orgf517e0c}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgca83550}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgfea6c40},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:org9f2ad75}

\begin{frame}[label={sec:org893761a}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org5c4fa96}]{Monoid}
...
\begin{frame}[label={sec:org72516ce}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:org4c5a749},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orgf8de122},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:orge72ec19}]{Commutativity}
\begin{itemize}
...
\begin{frame}[label={sec:orga0aed44}]{The most important question}
\pause
...
\section{Fibonacci}
\label{sec:orga51ab95}

\begin{frame}[label={sec:orgf7a1790}]{The Fibonacci numbers}
\begin{align*}
  F_0 &= 0 \\
  F_1 &= 1 \\
...
\begin{frame}[label={sec:org2734312},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org1fdd262},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgbe12aca},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgc0194bb},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org2cea0a8}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:orgba2d592},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:orgf55850b},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org5f11e55},fragile]{Power in terms of Monoid}
 \begin{minted}[]{dot}
digraph {
...
\begin{frame}[label={sec:orgf677076},fragile]{Power in terms of Monoid}
 \begin{minted}[]{dot}
digraph {
...
\begin{frame}[label={sec:org545bb3b},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orge63458d}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org3d4d712}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org6914c87},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orgaf01f44}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:orga7f97de},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:org13cbfe7}

\begin{frame}[label={sec:org13a9f2f},fragile]{Folds with Monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org5bf94c8},fragile]{Folds with Monoids}
 \begin{minted}[]{dot}
digraph {
...
\begin{frame}[label={sec:org9a810ce},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org2e4934a},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orge507195},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:org4ee7308},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org8a8c4a0},fragile]{Homework}
...
\section{Logger}
\label{sec:orgc409860}

\begin{frame}[label={sec:org22d1950}]{Many monoids}
...
\begin{frame}[label={sec:org2ae0313},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc212a33},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org08ce4c6},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org050c6b1},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org0f122e9},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org6d6b844},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:org5d3b419},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:org4011025}
\begin{frame}[label={sec:org642d4ed},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section*{References}
\label{sec:orgc7a2915}
\begin{frame}[label={sec:orga46289c}]{References}
\begin{itemize}
\item \url{https://people.csail.mit.edu/dnj/teaching/6898/papers/wadler88.pdf}
\end{itemize}
\end{frame}

\section{Questions?}
\label{sec:org29ffc02}

\section{Thank you very much!}
\label{sec:org3051958}
\end{document}
