...
\begin{frame}[label={sec:org1484f34},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgb792c8d},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org257d5ba},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:orgb7aaecf},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org667f803}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org577b583},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org1c2dbaf},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:orgf3fc215},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:orgc19531d}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org6944f7d},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org709d039},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org4de95c7},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org766a068}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:org162f7bb},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:org27f36c6},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:orgcea1e8a}]{What have we done?}
...
\begin{frame}[label={sec:orgc8dd1f0},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:orge72c139},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:orgf81078c}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org8b716cf}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:org5afa7a9}

\begin{frame}[label={sec:org07c0e79},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org88d0e0d}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:org71c17c5}]{Semigroup}
...
\begin{frame}[label={sec:org4bc7599},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:org5606353}]{Important!}
...
\begin{frame}[label={sec:org1f42386},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc9247f3},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgb7c05f1},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orgcec8c10}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org88f1659},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org72e62ef},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:orgb41be86}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgca01da1}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:org231be5c},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:org6359d9e}

\begin{frame}[label={sec:org0bc8694}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org1a22b1b}]{Monoid}
...
\begin{frame}[label={sec:orga77ccbe}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:orgd725d8f},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orgf37a377},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org615ecd9},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgaf8ca6b},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org1477954}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:orge3c84fe},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org8d44ab1}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org4446707}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org7dbc524}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org08849a6},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orge04192a}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org72af9c1}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org9dc97da},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org4ecd7b2}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:orga7bb1a3},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:org4ba0ffd}

\begin{frame}[label={sec:orgb1e1c27}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:orgd7f97d4}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org8a152ce},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org64bfe3f},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org3861f0a},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:org4ebee9a},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:orgfc4b86a},fragile]{Homework}
...
\section{Logger}
\label{sec:orgd6178c8}

\begin{frame}[label={sec:orgbd490d9}]{Many monoids}
...
\begin{frame}[label={sec:org504459f},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb5d749a},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org1926e77},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb56f092},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org151db8e},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org4cb29e0},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:org2c0194a},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:org6e2f319}
\begin{frame}[label={sec:org1ded6ae},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section{Questions?}
\label{sec:org5659c7f}

\section{Thank you very much!}
\label{sec:org37833c8}
\end{document}
