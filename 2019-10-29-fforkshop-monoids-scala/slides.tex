...
\section*{Intro}
\label{sec:org1c6930d}
\begin{frame}[label={sec:org8e27860}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:org8af8b16}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:org81492f1},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org8757e52}
\begin{frame}[label={sec:orge30c82e}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\begin{frame}[label={sec:org81f55ed},fragile]{Before we start}
 \begin{minted}[]{bash}
...
\label{sec:orgb343ad1}

\begin{frame}[label={sec:orgee3adc4}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:orgcf84435},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org91d72c1},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgfaa3e77},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org6658f0e},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:org4f378d6},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org884276a},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:orgdd08a58},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org4d2f1f9}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:orgb710f14},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org5910574},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org5e59998},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:orgfcc8ff8}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org5117562},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:orgf0e9be8},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org6d1f44c},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org7b9bffb}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orgda3b1e0},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orgd1595d6},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:orgd9a5e16}]{What have we done?}
...
\begin{frame}[label={sec:org804f999},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:orgab14d00},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org7a0026d}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org6fbb192}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:org09eb848}

\begin{frame}[label={sec:org6a03a90},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org7ca7e8a}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:orgb139275}]{Semigroup}
...
\begin{frame}[label={sec:org6dda18d},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orgae44877}]{Important!}
...
\begin{frame}[label={sec:org9a3b120},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org5df2638},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org046ecc4},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orge99d0d2}]{You're developer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgcf2dafe},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org8415572},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org5389c8a}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org31df757}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgbd3248a},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:org5ee578f}

\begin{frame}[label={sec:org737cd03}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:orgafabf5e}]{Monoid}
...
\begin{frame}[label={sec:org1a34272}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:org703b9d9},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:org6bb8352},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:org4b17b61}]{Commutativity}
\begin{itemize}
...
\begin{frame}[label={sec:org92abe7a}]{The most important question}
\pause
...
\section{Fibonacci}
\label{sec:org1e88e3c}

\begin{frame}[label={sec:orgc40dc31}]{The Fibonacci numbers}
\begin{align*}
  F_0 &= 0 \\
  F_1 &= 1 \\
...
\begin{frame}[label={sec:org286a284},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org51b117d},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org5798155},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org970052a},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org6a80484}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:org4890d07},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org0fa9e9a}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgdc536fd}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:orgda4d993}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org3947d48},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org3cc0974}]{Back to Fibonacci}
...
\begin{frame}[label={sec:orga3ce197}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org4ac4487},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org857b997}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:org99e61a6},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:org5924e67}

\begin{frame}[label={sec:orgce2a951}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org3c25922}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:orgae629fa},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org742f4a9},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orga9b0cfc},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:orgb1858b6},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org58963bd},fragile]{Homework}
...
\section{Logger}
\label{sec:org7adca9a}

\begin{frame}[label={sec:orgf013c39}]{Many monoids}
...
\begin{frame}[label={sec:orgcc39ce2},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org00291de},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc781c12},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb3ea488},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgde7f75d},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org0f0e714},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:org898993a},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:org5ac5219}
\begin{frame}[label={sec:org43470c9},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section{Questions?}
\label{sec:org717b5c2}

\section{Thank you very much!}
\label{sec:orga10a4a3}
\end{document}
