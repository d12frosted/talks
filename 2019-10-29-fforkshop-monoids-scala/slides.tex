...
\section*{Intro}
\label{sec:org0077c5c}
\begin{frame}[label={sec:orgefbc80b}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:org73e9ede}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:org4f8226c},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:orga97d0f6}
\begin{frame}[label={sec:orgee8fa1c}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\begin{frame}[label={sec:org963a272},fragile]{Before we start}
 \begin{minted}[]{bash}
...
\label{sec:org6c493a5}

\begin{frame}[label={sec:org5af1c46}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:org09c9136},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org3ba63cf},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgf9f4023},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org142cc92},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgcc14363},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org78bed99},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:org25c213a},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:orgae0b691}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org62aa378},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org3bc55f7},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org0bf6f85},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:orgcfaaf0b}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org8b10ccb},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:orgcf9ac12},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org2cab50d},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org8697681}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orgcdf514e},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orgda72b84},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org9cc0fb3}]{What have we done?}
...
\begin{frame}[label={sec:org4986dff},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:orgce41936},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org4719165}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org62251dc}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:org57049d5}

\begin{frame}[label={sec:org89d9b79},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orge268f5f}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:org05233cc}]{Semigroup}
...
\begin{frame}[label={sec:orgc0bb414},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:org13a7339}]{Important!}
...
\begin{frame}[label={sec:orgf07aaf2},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org026a072},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org2472a12},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orgb1aa582}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org159c2b9},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org6d8aa84},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org451118d}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org4871f8d}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:org97c284f},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:org2c49351}

\begin{frame}[label={sec:orgbe337a6}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org0cf3add}]{Monoid}
...
\begin{frame}[label={sec:orgdea4d98}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:orge7189e1},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orgba40b2f},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:org02d1291}]{Commutativity}
\begin{itemize}
...
\begin{frame}[label={sec:org21bda5e}]{The most important question}
\pause
...
\section{Fibonacci}
\label{sec:orgd1b9837}

\begin{frame}[label={sec:org5608161}]{The Fibonacci numbers}
\begin{align*}
  F_0 &= 0 \\
  F_1 &= 1 \\
...
\begin{frame}[label={sec:orgc4393fe},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgd845067},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org0ed75cf},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgc4ddf3b},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org89b404e}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:orge406e0e},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org8722659}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgb1b59d6}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org96d6e18}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:orgb5aef54},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgc6473f5}]{Back to Fibonacci}
...
\begin{frame}[label={sec:orgfbe77c6}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org2f331f7},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org430f3a1}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:orgecee671},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:org95bd820}

\begin{frame}[label={sec:orgc66b0bd}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org83476cb}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org2721821},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org0b6ef69},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org8567543},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:orge987140},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:orgab39746},fragile]{Homework}
...
\section{Logger}
\label{sec:org553d113}

\begin{frame}[label={sec:org821f277}]{Many monoids}
...
\begin{frame}[label={sec:orgb98c272},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org441e2a3},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org14461fd},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org677c101},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgd26535b},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org5c343b2},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb69ad34},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:org3854393}
\begin{frame}[label={sec:orgc558c39},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section{Questions?}
\label{sec:org7026087}

\section{Thank you very much!}
\label{sec:org357b3eb}
\end{document}
