...
\section*{Intro}
\label{sec:org715101a}
\begin{frame}[label={sec:org6a0a566}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:orgdac6659}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:orgc68b01d},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org4b05ad5}
\begin{frame}[label={sec:orgd946e83}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\begin{frame}[label={sec:orgda80a60},fragile]{Before we start}
 \begin{minted}[]{bash}
...
\label{sec:org0c004e9}

\begin{frame}[label={sec:org5d76f60}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:orgf92a4a1},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org30f2b19},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org797ae2a},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org99844b5},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:org3ba93d0},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org1782604},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:orgf6278a2},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org3d24446}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org7c4dcbc},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org3b6dbb8},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org137a1d9},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org0a35489}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:orgc3283f5},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:orgd1442fd},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org203a534},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:orgda115b2}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orgab792a1},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orgf0a2421},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org48fe7e7}]{What have we done?}
...
\begin{frame}[label={sec:orgb2312ce},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:org96f2f82},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:orgd5c2c4c}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org7b040f6}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:org9809f30}

\begin{frame}[label={sec:org754c298},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgaa00998}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:orgf178f43}]{Semigroup}
...
\begin{frame}[label={sec:org52269bd},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:org5bbf94a}]{Important!}
...
\begin{frame}[label={sec:org26d6c5b},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:org6cf7c93},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org1074db2},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orgaafda5c}]{You're developer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org1a75698},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orga635f46},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org66abf47}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org0955651}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:org8827765},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:orgdf16be6}

\begin{frame}[label={sec:org29938f2}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:orgf93927c}]{Monoid}
...
\begin{frame}[label={sec:org309b88a}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:orgcbbcf79},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orgadd32fd},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:org4580f0a}]{Commutativity}
\begin{itemize}
...
\begin{frame}[label={sec:org5a7ccdb}]{The most important question}
\pause
...
\section{Fibonacci}
\label{sec:orga4dc35e}

\begin{frame}[label={sec:org7f93e8a}]{The Fibonacci numbers}
\begin{align*}
  F_0 &= 0 \\
  F_1 &= 1 \\
...
\begin{frame}[label={sec:org8a22a19},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org395ccac},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgf7c4a00},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgc9c632a},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org7757a53}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:orgdcbf654},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:orgfc11c6e}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgb9d8276}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:orge09deb8}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:orgfdcacd1},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org7d99caf}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org299b9a1}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:orgd0ea6d4},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org7f07e7f}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:orga6d6ddd},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:org3e7f6c6}

\begin{frame}[label={sec:orgda5574e}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:orgdbd46b6}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:org33ad29c},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org3e13113},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orga36db46},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:orgd2c3e22},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org51b4b2a},fragile]{Homework}
...
\section{Logger}
\label{sec:org6ab3b3d}

\begin{frame}[label={sec:org92181cf}]{Many monoids}
...
\begin{frame}[label={sec:orgbb37a94},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org637298a},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:orgab46c85},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org0a63566},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org1ddcf34},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgcc58b5a},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:org47ed76e},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:orga183928}
\begin{frame}[label={sec:org5b66fd3},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section{Questions?}
\label{sec:org968d850}

\section{Thank you very much!}
\label{sec:orgbfff4a5}
\end{document}
