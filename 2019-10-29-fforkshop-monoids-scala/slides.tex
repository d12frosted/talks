...
\begin{frame}[label={sec:org40c645e},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orgc69d076},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgb3d1b8e},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:org17c97b7},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org0416717}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org5a26aa7},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org0bca982},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org0929062},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org53d0ba1}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org033c613},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org398073f},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org22128e3},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org54b016e}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orgf14f09d},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:orga3e152d},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org1e989f1}]{What have we done?}
...
\begin{frame}[label={sec:orgacb0c9e},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:orgcfc2bba},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:orgf91070c}]{Type classes: extensibility}
...
\begin{frame}[label={sec:orgf4c3f84}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:orgd4c2c9d}

\begin{frame}[label={sec:orgc1b1b73},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgd0dd7ef}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:org9f610e9}]{Semigroup}
...
\begin{frame}[label={sec:org240c9a6},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orgfcca760}]{Important!}
...
\begin{frame}[label={sec:orga5523d5},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:orge8ee960},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org10c8e94},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:org0b9b156}]{You're programmer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:orge789c7d},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org3bc4890},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org2e3158e}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:orga0d8526}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgb3008dc},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:orgf220565}

\begin{frame}[label={sec:org702ba87}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org793e9aa}]{Monoid}
...
\begin{frame}[label={sec:org4049e47}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:orga1206fd},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\section{Fibonacci}
\label{sec:org7653439}

\begin{frame}[label={sec:orgb0093be}]{The Fibonacci numbers}
\begin{align*}
  F_0 &= 0 \\
  F_1 &= 1 \\
...
\begin{frame}[label={sec:org718d758},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgb54662b},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org339aaa7},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orge5c1a76},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:org97f061f}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:orgc044622},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org663604d}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgc847fe8}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org305fb8e}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org952614a},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org9ce6efb}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org7eeb9b4}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org4f0c552},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org1545f28}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:org5c0d66e},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:orgdf3bfb6}

\begin{frame}[label={sec:orgc85aa29}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org01efdb7}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:orgc2c4fe5},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org7ee2455},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb6691a5},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:org106ac81},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org4ffb18d},fragile]{Homework}
...
\section{Logger}
\label{sec:orge7cab00}

\begin{frame}[label={sec:org40670f4}]{Many monoids}
...
\begin{frame}[label={sec:org0692c26},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org95ceeda},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org48558d9},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org9405e1d},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgeca2399},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org82d2c89},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc6f0c18},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:orgb6861c1}
\begin{frame}[label={sec:org2d9cf76},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section{Questions?}
\label{sec:org2963355}

\section{Thank you very much!}
\label{sec:orgc31c8aa}
\end{document}
