...
\section*{Intro}
\label{sec:org695bdc9}
\begin{frame}[label={sec:org7fc6a4a}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:org8321af7}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:orgcd887a3},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org1501183}
\begin{frame}[label={sec:org8d7401b}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\begin{frame}[label={sec:org9a7fa66},fragile]{Before we start}
 \begin{minted}[]{bash}
...
\label{sec:org5c03a81}

\begin{frame}[label={sec:org91e4754}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:org09bba5a},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org6c666bd},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgf5bc703},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:orge6dca81},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orge14ed75},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:org43c7eef},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:org48e5dec},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:orgd875913}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:orgeb6a68a},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:org0694d48},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:orgc328ad5},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:orgc81ce80}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org0ca9b97},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org109bbde},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:orgdcc70da},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org52ecea5}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orgc26bb86},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:org584495f},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:org3300fd4}]{What have we done?}
...
\begin{frame}[label={sec:orgc4e2cdf},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:org298180f},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org7b49302}]{Type classes: extensibility}
...
\begin{frame}[label={sec:orgf28be55}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:orgbe0d8c4}

\begin{frame}[label={sec:org0c709cc},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgf7d565d}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:orga7c1e3a}]{Semigroup}
...
\begin{frame}[label={sec:org0b7370a},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:orgfd93bf7}]{Important!}
...
\begin{frame}[label={sec:orgbebf74f},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:orgba8c7d2},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgcda9f67},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orgfa8bf16}]{You're developer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org0195663},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org98e3ddc},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org69d0dd2}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgcb97b93}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgf041a66},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:org64bedf9}

\begin{frame}[label={sec:org9c631b5}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:org7dd8227}]{Monoid}
...
\begin{frame}[label={sec:org3753668}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:orgfec13e2},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:orgbdacdc5},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:org33a13a3}]{Commutativity}
\begin{itemize}
...
\begin{frame}[label={sec:org12e1a97}]{The most important question}
\pause
...
\section{Fibonacci}
\label{sec:org07bbdfd}

\begin{frame}[label={sec:org459778e}]{The Fibonacci numbers}
\begin{align*}
  F_0 &= 0 \\
  F_1 &= 1 \\
...
\begin{frame}[label={sec:orgd49af87},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org46d1303},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org80dcb97},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org6cf7170},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:orgbd26aef}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:org865c297},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:org09debe4}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgcb1a58f}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org2557e1d}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org21871c2},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:org8fe11c1}]{Back to Fibonacci}
...
\begin{frame}[label={sec:orgb9c1b6f}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org6e49d0f},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org927dc5e}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:orgd12b924},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:orgcfa3394}

\begin{frame}[label={sec:org75ba3c4}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:org4025b52}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:orge245485},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org7f9e02a},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orga80c51b},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:org1ea9012},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org6c8978d},fragile]{Homework}
...
\section{Logger}
\label{sec:org8d62ddf}

\begin{frame}[label={sec:orgbe429e3}]{Many monoids}
...
\begin{frame}[label={sec:org9baab20},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:org2501a54},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org2b6ea01},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:orgbadecd3},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org4629177},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org5df9e30},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:orgf882afe},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:orge498015}
\begin{frame}[label={sec:orgf78fa43},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section{Questions?}
\label{sec:orge2be9e4}

\section{Thank you very much!}
\label{sec:org8ea7482}
\end{document}
