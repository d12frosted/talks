...
\section*{Intro}
\label{sec:orgc782c2b}
\begin{frame}[label={sec:org5a2fc51}]{About us}
\vspace*{20px}
...
\begin{frame}[label={sec:org59aa1ba}]{About the Forkshop}
\begin{itemize}
...
\begin{frame}[label={sec:orgeb4c4cc},fragile]{Whys}
 \begin{itemize}
...
\section*{Today}
\label{sec:org3fa8766}
\begin{frame}[label={sec:orgaff675e}]{Agenda}
\begin{itemize}
\item Type classes
\item Semigroups
\item Monoids
...
\begin{frame}[label={sec:orgbaa40b1},fragile]{Before we start}
 \begin{minted}[]{bash}
...
\label{sec:orgfaa0a8c}

\begin{frame}[label={sec:orgcbb978b}]{Application definition}
\begin{itemize}
...
\begin{frame}[label={sec:org90ea700},fragile]{Meet the hero}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org76cd8c8},fragile]{Every hero needs a monster}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgd34197f},fragile]{Game}
 \begin{columns}
...
\begin{frame}[label={sec:org2698329},fragile]{Introducing abstractions}
 \begin{columns}
...
\begin{frame}[label={sec:orge3dae3e},fragile]{Game with trait}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
...
\begin{frame}[label={sec:orga67b6d5},fragile]{Here comes the cockatrice}
 \begin{columns}
...
\begin{frame}[label={sec:orgba1d87a},fragile]{Shawarma to the rescue}
 \begin{columns}
\begin{column}{0.25\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org444cc75}]{Calm down and reevaluate our goal}
\begin{itemize}
...
\begin{frame}[label={sec:org13b18cf},fragile]{\texttt{trait} + wrapper: abstraction}
...
\begin{frame}[label={sec:orgd658265},fragile]{\texttt{trait} + wrapper: composition}
...
\begin{frame}[label={sec:org32caa10},fragile]{\texttt{trait} + wrapper: extensibility}
...
\begin{frame}[label={sec:org554e53b}]{You know where it’s going to, right?}
\pause
...
\begin{frame}[label={sec:org23bbe45},fragile]{Dividing data and behaviour}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible {
...
\begin{frame}[label={sec:org86c3a99},fragile]{Usage}
 \begin{minted}[]{scala}
// Define new trait
trait Introducible[A] {
  def introduce(a: A): String
...
\begin{frame}[label={sec:org95a6e09},fragile]{External types? Pff\ldots{}}
 \begin{minted}[]{scala}
import io.proprietary.monsters.cockatrice._
...
\begin{frame}[label={sec:org71a332f}]{But passing implementation around is\ldots{}}
\begin{center}
...
\begin{frame}[label={sec:orgcd233da},fragile]{So implicits :(}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
object Hero {
...
\begin{frame}[label={sec:org94ce7d4},fragile]{Summoning the summoner}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{minted}[]{scala}
trait Introducible[A] {
...
\begin{frame}[label={sec:orgf6f52e1}]{What have we done?}
...
\begin{frame}[label={sec:orgdcb94a8},fragile]{Type classes: abstraction}
 Abstraction holds. Proof is the \texttt{introduce} function itself.
...
\begin{frame}[label={sec:orgc0d3dea},fragile]{Type classes: composition}
...
\begin{frame}[label={sec:org7a45481}]{Type classes: extensibility}
...
\begin{frame}[label={sec:org12eaf9f}]{Type classes: final thoughts}
\begin{enumerate}
...
\section{Semigroup}
\label{sec:org75875f7}

\begin{frame}[label={sec:orgd85a430},fragile]{Time for a quiz!}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:orgf78f3ab}]{Associativity}
\begin{enumerate}
...
\begin{frame}[label={sec:orgcb92fec}]{Semigroup}
...
\begin{frame}[label={sec:org86555ce},fragile]{But it’s not that scary}
 \begin{columns}
...
\begin{frame}[label={sec:org17af8ed}]{Important!}
...
\begin{frame}[label={sec:org1b6bbcb},fragile]{What is law?}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc859310},fragile]{Instance example}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org8e6c9d6},fragile]{Checking laws - \sout{pen and paper} in comments}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
    override def combine(x: String, y: String): String = x + y
...
\begin{frame}[label={sec:orga1ed4b2}]{You're developer after all}
\begin{center}
\includegraphics[height=7cm]{images/you-re-programmer.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org00a52b1},fragile]{Question on the interview: property based testing}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:orgbd71854},fragile]{More than one valid instance}
 \begin{minted}[]{scala}
package object implicits {
  implicit val stringSemigroup: Semigroup[String] = new Semigroup[String] {
...
\begin{frame}[label={sec:org2e48e2c}]{More examples}
\begin{itemize}
...
\begin{frame}[label={sec:org4344612}]{Contra-examples}
\begin{itemize}
...
\begin{frame}[label={sec:orgcce3a53},fragile]{Coding time}
 \begin{enumerate}
...
\section{Monoid}
\label{sec:orga727eed}

\begin{frame}[label={sec:org91a1324}]{Monoid}
\begin{itemize}
...
\begin{frame}[label={sec:orgee18174}]{Monoid}
...
\begin{frame}[label={sec:org1134c86}]{Monoid}
Back to fancy words.
...
\begin{frame}[label={sec:org237b786},fragile]{Again, it's not that scary}
 \begin{minted}[]{scala}
package object typeclass {
...
\begin{frame}[label={sec:org4b920a6},fragile]{Examples}
 \begin{itemize}
...
\begin{frame}[label={sec:orgb1cb1f0}]{Commutativity}
\begin{itemize}
...
\begin{frame}[label={sec:orgd92daf2}]{The most important question}
\pause
...
\section{Fibonacci}
\label{sec:orgccd9d06}

\begin{frame}[label={sec:org7b38d2b}]{The Fibonacci numbers}
\begin{align*}
  F_0 &= 0 \\
  F_1 &= 1 \\
...
\begin{frame}[label={sec:orge138779},fragile]{Solution}
 \begin{columns}
\begin{column}[t]{0.5\columnwidth}
...
\begin{frame}[label={sec:org3a58069},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org0876e5c},fragile]{Two folds}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org8964ae9},fragile]{Two folds}
 \begin{itemize}
...
\begin{frame}[label={sec:orge064811}]{Two folds}
\begin{columns}
\begin{column}[t]{0.5\columnwidth}
\begin{block}{foldl}
$$+ : B \rightarrow A \rightarrow B$$
...
\begin{frame}[label={sec:orgfc28f64},fragile]{What Monoid gives us}
 \begin{itemize}
...
\begin{frame}[label={sec:orgcecc849}]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orgcfeb7a7}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:orgb025c42}]{Power in terms of Monoid}
\begin{center}
...
\begin{frame}[label={sec:org43c700c},fragile]{Power in terms of Monoid}
...
\begin{frame}[label={sec:orga353337}]{Back to Fibonacci}
...
\begin{frame}[label={sec:org1f1f7a5}]{Back to Fibonacci}
\begin{equation*}
  \begin{pmatrix}
    F_{n+1} & F_n \\
    F_n & F_{n-1}
...
\begin{frame}[label={sec:org2650d06},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:org5d56327}]{Profiling results}
\begin{center}
\begin{tabular}{rrr}
N & Matrix & Tailrec\\
\hline
...
\begin{frame}[label={sec:orgfeb6fd3},fragile]{Outcome}
 \begin{itemize}
...
\section{Books}
\label{sec:orgbbfc21e}

\begin{frame}[label={sec:orgf747bb7}]{Folds with Monoids}
\begin{itemize}
...
\begin{frame}[label={sec:orgfd3fb7d}]{Folds with Monoids}
\begin{center}
...
\begin{frame}[label={sec:orga9f0e55},fragile]{MapReduce}
 \begin{itemize}
...
\begin{frame}[label={sec:org8038be1},fragile]{Coding time}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc27d53b},fragile]{Benchmarks}
 \begin{verbatim}
Par
duration = 65633 ms
...
\begin{frame}[label={sec:org6d0272b},fragile]{Outcome}
 \begin{itemize}
...
\begin{frame}[label={sec:org59ba65c},fragile]{Homework}
...
\section{Logger}
\label{sec:org1846c93}

\begin{frame}[label={sec:org1a44453}]{Many monoids}
...
\begin{frame}[label={sec:orgb6d5c4b},fragile]{Let's start with some wrappers (pun intended)}
 \begin{itemize}
...
\begin{frame}[label={sec:orgae50e76},fragile]{Wrappers of monoids are monoids}
 \begin{itemize}
...
\begin{frame}[label={sec:org4023f0a},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org1098864},fragile]{Logger}
 \begin{minted}[]{scala}
...
\begin{frame}[label={sec:org8712655},fragile]{Logger}
 \begin{itemize}
...
\begin{frame}[label={sec:org02e9a76},fragile]{Bonus questions}
 \begin{itemize}
...
\begin{frame}[label={sec:org74cdfbb},fragile]{Outcome}
 \begin{itemize}
...
\section*{Final words}
\label{sec:orgea38847}
\begin{frame}[label={sec:orgac41374},fragile]{Recap (recup?)}
 \begin{itemize}
...
\section{Questions?}
\label{sec:org0ec3856}

\section{Thank you very much!}
\label{sec:org019b3f0}
\end{document}
