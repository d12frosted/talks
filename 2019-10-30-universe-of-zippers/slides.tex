...
\section*{Intro}
\label{sec:orgc818433}
\begin{frame}[label={sec:orgcf9afad}]{About me}
\begin{columns}
...
\begin{frame}[label={sec:org6eb9dab}]{Agenda}
\begin{itemize}
...
\label{sec:org62aaeca}

\begin{frame}[label={sec:orge2a9909}]{Cellular Automaton}
...
\begin{frame}[label={sec:org79d7f51}]{Many grids}
\begin{enumerate}
...
\begin{frame}[label={sec:org8c85079},t]{Elementary Cellular Automata}
\begin{onlyenv}<1->
...
\begin{frame}[label={sec:org4486427},t]{Evolution example}
\begin{onlyenv}<1-3>
...
\begin{frame}[label={sec:orgf1cba76}]{ECA: specifying the rule}
\begin{columns}
...
\begin{frame}[label={sec:orgf9d54e5}]{ECA: rule example}
\begin{columns}
\begin{column}{0.2\columnwidth}
\begin{center}
\begin{tabular}{rrrr}
...
\begin{frame}[label={sec:orgf298ddc}]{There is more}
\begin{itemize}
...
\begin{frame}[label={sec:orgada2030},fragile]{Universe}
...
\begin{frame}[label={sec:org90aab98}]{Reversed left?}
\begin{itemize}
...
\begin{frame}[label={sec:org0dd65a2},fragile]{\texttt{fromList}}
 \begin{minted}[]{haskell}
fromList :: a -> [a] -> U a
...
\begin{frame}[label={sec:org188520f},fragile]{\texttt{extract}}
 \begin{minted}[]{haskell}
extract :: U a -> a
extract (U _ a _) = a
\end{minted}
...
\begin{frame}[label={sec:orgd790308},fragile]{\texttt{Functor}}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org036c126}]{Movement}
\pause
...
\begin{frame}[label={sec:org258e4bb},fragile]{Moving left}
 \begin{minted}[]{haskell}
left :: U a -> U a
...
\begin{frame}[label={sec:org3f0ed66},fragile]{Moving right}
 \begin{minted}[]{haskell}
right :: U a -> U a
...
\begin{frame}[label={sec:org6cf060c},fragile]{Non-totality}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgad4a870},fragile]{Defining ECA}
 \begin{minted}[]{haskell}
data Cell
  = Dead
  | Alive
...
\begin{frame}[label={sec:org0ebd060},fragile]{Defining rule}
...
\begin{frame}[label={sec:orgd004648},fragile]{Rule 110}
 \begin{minted}[]{haskell}
rule110 :: U Cell -> Cell
...
\begin{frame}[label={sec:orgd5a37f8},fragile]{Possible solution}
 \begin{enumerate}
...
\begin{frame}[label={sec:orgca0532e}]{M-x doctor}
\begin{verse}
...
\begin{frame}[label={sec:org8cac9cb},fragile]{All possible shifts}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
duplicate :: U a -> U (U a)
\end{minted}
...
\begin{frame}[label={sec:org9f39ec3}]{All possible shifts}
\begin{gather*}
...
\begin{frame}[label={sec:org055d4ed}]{Where's the focus?}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:orgff68dde}]{Where's the focus?}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:orgcd12824}]{Where's the focus}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:org048685d},fragile,t]{\texttt{extend}}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
duplicate :: U a
          -> U (U a)
...
\begin{frame}[label={sec:org88f79d2},fragile]{Iteration}
 \begin{minted}[,escapeinside=||,mathescape=true]{haskell}
...
\begin{frame}[label={sec:orga8ad0da},fragile]{(not so) Final words}
 \begin{itemize}
...
\label{sec:orgbd87735}

\begin{frame}[label={sec:org0dbe496}]{Definition}
...
\begin{frame}[label={sec:orge002296}]{Rules}
...
\begin{frame}[label={sec:orgc26db13}]{Example of simulation (1)}
\begin{footnotesize}
  \begin{equation*}
    \begingroup
    \setlength\arraycolsep{0.25pt}
...
\begin{frame}[label={sec:orga1fa405}]{Example of simulation (2)}
\begin{equation*}
  \begingroup
  \setlength\arraycolsep{0.25pt}
  \renewcommand{\arraystretch}{0.5}
...
\begin{frame}[label={sec:orga19ea1e}]{Example of simulation (3)}
\begin{tiny}
  \begin{equation*}
    \begingroup
    \setlength\arraycolsep{0.25pt}
...
\begin{frame}[label={sec:orgb19ba50}]{\emph{untitled copy (1)(2) - final}}
\begin{onlyenv}<1>
iTunes anyone?
\end{onlyenv}
...
\begin{frame}[label={sec:org5084273},fragile]{Rolling out U2}
 \begin{minted}[]{haskell}
type U2 a = U (U a)
\end{minted}
\end{frame}

\begin{frame}[label={sec:orgab3b361},fragile]{Moving around in the \sout{complex} Universe}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org22e381b},fragile]{Game of Life rules}
 \begin{minted}[]{haskell}
rule :: U (U Cell) -> Cell
rule u | ns == 3   = Alive
...
\begin{frame}[label={sec:org8a384a1},fragile]{Was easy, right?}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgfb557b8},fragile]{Extending 2D Universe (1)}
 \begin{minted}[]{haskell}
> :t extend
...
\begin{frame}[label={sec:org509cda9},fragile]{Extending 2D Universe (2)}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orga87d7e2},fragile]{Actually extending 2D Universe}
 \begin{minted}[]{haskell}
newtype U2 a
  = U2
  { getUniverse :: U (U a)
...
\begin{frame}[label={sec:orge52e009},fragile]{Works}
 \begin{minted}[,escapeinside=||,mathescape=true]{haskell}
...
\begin{frame}[label={sec:orgc39e947}]{(not so) Final words}
\begin{itemize}
...
\label{sec:org404fd91}

\begin{frame}[label={sec:org4c8ba81}]{Definition}
\pause

Do you really need a definition of Bomberman?
\end{frame}

\begin{frame}[label={sec:org4994800}]{Definition}
\begin{center}
\includegraphics[width=.9\linewidth]{images/bomberman-1.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org521e265}]{But we have a different one}
\begin{center}
\includegraphics[height=8cm]{images/bomberman-2.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org7032abf}]{Bells and whistles}
\begin{center}
\includegraphics[height=8cm]{images/bomberman-3.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org4fb6d42},fragile]{Reusing U2}
 \begin{minted}[]{haskell}
data Cell
  = Bomberman
  | BombBomberman
...
\begin{frame}[label={sec:orgfb39cf7},fragile]{What is bot?}
 \begin{minted}[]{haskell}
bot :: ( MonadIO m
       , MonadReader r m
       , HasLogFunc r
...
\begin{frame}[label={sec:org9324180}]{What we care about}
\begin{itemize}
...
\begin{frame}[label={sec:org116c6ff},fragile]{Fatum of the Universe}
 \begin{minted}[]{haskell}
data MarkedCell
  = MarkedCell
  { getCell        :: !Cell
...
\begin{frame}[label={sec:orga459ede},fragile]{Fatum of the Universe}
 \begin{minted}[]{haskell}
> :t extend
...
\begin{frame}[label={sec:orga994411},fragile,t]{The bot}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
bot :: ( MonadIO m
       , MonadReader r m
...
\begin{frame}[label={sec:orgc81a7c9}]{Danger! Live demo}
\emph{Live demos are subject to fail. Accept and forgive. Move on.}
\end{frame}

\begin{frame}[label={sec:org9a77e92}]{(not so) Final words}
\begin{itemize}
...
\section{Finale}
\label{sec:org230af3b}

\begin{frame}[label={sec:orgb038135},fragile]{Commonalities (1)}
 \begin{minted}[]{haskell}
extract :: U a  -> a
extract :: U2 a -> a
\end{minted}
...
\begin{frame}[label={sec:org3273849},fragile]{Commonalities (2)}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgde28bf0},fragile]{Comonad}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org2b6f9c8},fragile]{Co-monad}
 \begin{columns}
\begin{column}{0.45\columnwidth}
\begin{minted}[]{haskell}
extract :: f a -> a
...
\begin{frame}[label={sec:org35d4a58},fragile]{Universe is Comonad}
...
\begin{frame}[label={sec:org2cf72b4},fragile]{Store}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org24c2ab4}]{Many uses}
\begin{itemize}
\item <1-> Instances
\begin{itemize}
...
\begin{frame}[label={sec:orgaca93b4}]{Final words}
\begin{itemize}
...
\begin{frame}[label={sec:org4dccaa7}]{Grab the code}
\url{https://gitlab.com/d12frosted/talks}
\end{frame}

\section*{References}
\label{sec:orge4c7814}
\begin{frame}[label={sec:org36d149a}]{References}
\begin{itemize}
...
\section{Questions?}
\label{sec:org8381449}
\section{Thank you}
\label{sec:org37ec166}
\end{document}
