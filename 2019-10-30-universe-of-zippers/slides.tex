...
\section*{Intro}
\label{sec:org999dfa3}
\begin{frame}[label={sec:org559afb4}]{About me}
\begin{columns}
...
\begin{frame}[label={sec:orge3ef811}]{Agenda}
\begin{itemize}
...
\label{sec:orga54a25a}

\begin{frame}[label={sec:org4911355}]{Cellular Automaton}
...
\begin{frame}[label={sec:orgae1fa4c}]{Many grids}
\begin{enumerate}
...
\begin{frame}[label={sec:org1bcf2d7},t]{Elementary Cellular Automata}
\begin{onlyenv}<1->
...
\begin{frame}[label={sec:org227b9e1},t]{Evolution example}
\begin{onlyenv}<1-3>
...
\begin{frame}[label={sec:org1905c70}]{ECA: specifying the rule}
\begin{columns}
...
\begin{frame}[label={sec:org8139e50}]{ECA: rule example}
\begin{columns}
\begin{column}{0.2\columnwidth}
\begin{center}
\begin{tabular}{rrrr}
...
\begin{frame}[label={sec:org562a194}]{There is more}
\begin{itemize}
...
\begin{frame}[label={sec:org5caf937},fragile]{Universe}
...
\begin{frame}[label={sec:org1a7cd6c}]{Reversed left?}
\begin{itemize}
...
\begin{frame}[label={sec:org2899873},fragile]{\texttt{fromList}}
 \begin{minted}[]{haskell}
fromList :: a -> [a] -> U a
...
\begin{frame}[label={sec:org40f8d75},fragile]{\texttt{extract}}
 \begin{minted}[]{haskell}
extract :: U a -> a
extract (U _ a _) = a
\end{minted}
...
\begin{frame}[label={sec:org8573ae0},fragile]{\texttt{Functor}}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org040ef2d}]{Movement}
\pause
...
\begin{frame}[label={sec:org091bb02},fragile]{Moving left}
 \begin{minted}[]{haskell}
left :: U a -> U a
...
\begin{frame}[label={sec:org4e08534},fragile]{Moving right}
 \begin{minted}[]{haskell}
right :: U a -> U a
...
\begin{frame}[label={sec:org22822ed},fragile]{Non-totality}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgd734b57},fragile]{Defining ECA}
 \begin{minted}[]{haskell}
data Cell
  = Dead
  | Alive
...
\begin{frame}[label={sec:orgfe0dd2b},fragile]{Defining rule}
...
\begin{frame}[label={sec:org4b5be6f},fragile]{Rule 110}
 \begin{minted}[]{haskell}
rule110 :: U Cell -> Cell
...
\begin{frame}[label={sec:orgc2b1cab},fragile]{Possible solution}
 \begin{enumerate}
...
\begin{frame}[label={sec:org6e5563b}]{M-x doctor}
\begin{verse}
...
\begin{frame}[label={sec:orgcb95fbe},fragile]{All possible shifts}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
duplicate :: U a -> U (U a)
\end{minted}
...
\begin{frame}[label={sec:orgce8b5ca}]{All possible shifts}
\begin{gather*}
...
\begin{frame}[label={sec:org45922c3}]{Where's the focus?}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:orga6032bc}]{Where's the focus?}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:orgd39bdc5}]{Where's the focus}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:org51b075b},fragile,t]{\texttt{extend}}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
duplicate :: U a
          -> U (U a)
...
\begin{frame}[label={sec:org228bb4f},fragile]{Iteration}
 \begin{minted}[,escapeinside=||,mathescape=true]{haskell}
...
\begin{frame}[label={sec:org88d17b6}]{Zipper}
...
\begin{frame}[label={sec:org6a5c175},fragile]{(not so) Final words}
 \begin{itemize}
...
\label{sec:org3a219d5}

\begin{frame}[label={sec:orgd6d3889}]{Definition}
...
\begin{frame}[label={sec:org4725ee2}]{Rules}
...
\begin{frame}[label={sec:org4e3c28b}]{Example of simulation (1)}
\begin{footnotesize}
  \begin{equation*}
    \begingroup
    \setlength\arraycolsep{0.25pt}
...
\begin{frame}[label={sec:orgfdb3a89}]{Example of simulation (2)}
\begin{equation*}
  \begingroup
  \setlength\arraycolsep{0.25pt}
  \renewcommand{\arraystretch}{0.5}
...
\begin{frame}[label={sec:orgfab7e4c}]{Example of simulation (3)}
\begin{tiny}
  \begin{equation*}
    \begingroup
    \setlength\arraycolsep{0.25pt}
...
\begin{frame}[label={sec:org46c598c}]{\emph{untitled copy (1)(2) - final}}
\begin{onlyenv}<1>
iTunes anyone?
\end{onlyenv}
...
\begin{frame}[label={sec:orgb730d00},fragile]{Rolling out U2}
 \begin{minted}[]{haskell}
type U2 a = U (U a)
\end{minted}
\end{frame}

\begin{frame}[label={sec:org49cb01b},fragile]{Moving around in the \sout{complex} Universe}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgd32c36e},fragile]{Game of Life rules}
 \begin{minted}[]{haskell}
rule :: U (U Cell) -> Cell
rule u | ns == 3   = Alive
...
\begin{frame}[label={sec:org13a4a90},fragile]{Was easy, right?}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org447584c},fragile]{Extending 2D Universe (1)}
 \begin{minted}[]{haskell}
> :t extend
...
\begin{frame}[label={sec:orgb17fb11},fragile]{Extending 2D Universe (2)}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org9408a22},fragile]{Actually extending 2D Universe}
 \begin{minted}[]{haskell}
newtype U2 a
  = U2
  { getUniverse :: U (U a)
...
\begin{frame}[label={sec:org09c835a},fragile]{Works}
 \begin{minted}[,escapeinside=||,mathescape=true]{haskell}
...
\begin{frame}[label={sec:org44eab3b}]{(not so) Final words}
\begin{itemize}
...
\label{sec:org53f65bc}

\begin{frame}[label={sec:org8f41a0b}]{Definition}
\pause

Do you really need a definition of Bomberman?
\end{frame}

\begin{frame}[label={sec:org4803321}]{Definition}
\begin{center}
\includegraphics[width=.9\linewidth]{images/bomberman-1.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgdf4db9e}]{But we have a different one}
\begin{center}
\includegraphics[height=8cm]{images/bomberman-2.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org54ebfb0}]{Bells and whistles}
\begin{center}
\includegraphics[height=8cm]{images/bomberman-3.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org38df678},fragile]{Reusing U2}
 \begin{minted}[]{haskell}
data Cell
  = Bomberman
  | BombBomberman
...
\begin{frame}[label={sec:orge0f72b7},fragile]{What is bot?}
 \begin{minted}[]{haskell}
bot :: ( MonadIO m
       , MonadReader r m
       , HasLogFunc r
...
\begin{frame}[label={sec:org87ebe83}]{What we care about}
\begin{itemize}
...
\begin{frame}[label={sec:orgbb20a6b},fragile]{Fatum of the Universe}
 \begin{minted}[]{haskell}
data MarkedCell
  = MarkedCell
  { getCell        :: !Cell
...
\begin{frame}[label={sec:orgfe08f3d},fragile]{Fatum of the Universe}
 \begin{minted}[]{haskell}
> :t extend
...
\begin{frame}[label={sec:org9b88b28},fragile,t]{The bot}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
bot :: ( MonadIO m
       , MonadReader r m
...
\begin{frame}[label={sec:org590982a}]{Danger! Live demo}
\emph{Live demos are subject to fail. Accept and forgive. Move on.}
\end{frame}

\begin{frame}[label={sec:org9453c4a}]{(not so) Final words}
\begin{itemize}
...
\section{Finale}
\label{sec:org9a36583}

\begin{frame}[label={sec:orgff57b05},fragile]{Commonalities (1)}
 \begin{minted}[]{haskell}
extract :: U a  -> a
extract :: U2 a -> a
\end{minted}
...
\begin{frame}[label={sec:org0ab8425},fragile]{Commonalities (2)}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgad563fd},fragile]{Comonad}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org28747c1},fragile]{Co-monad}
 \begin{columns}
\begin{column}{0.45\columnwidth}
\begin{minted}[]{haskell}
extract :: f a -> a
...
\begin{frame}[label={sec:org6726f88},fragile]{Universe is Comonad}
