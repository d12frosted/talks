...
\section*{Intro}
\label{sec:org3b1818f}
\begin{frame}[label={sec:org5fba9cc}]{About me}
\begin{columns}
...
\begin{frame}[label={sec:orge45cc98}]{Agenda}
\begin{itemize}
...
\label{sec:org53ed87a}

\begin{frame}[label={sec:org43d2a11}]{Cellular Automaton}
...
\begin{frame}[label={sec:org0d7cdab}]{Many grids}
\begin{enumerate}
...
\begin{frame}[label={sec:org007a6f0},t]{Elementary Cellular Automata}
\begin{onlyenv}<1->
...
\begin{frame}[label={sec:org922567c},t]{Evolution example}
\begin{onlyenv}<1-3>
...
\begin{frame}[label={sec:orgf4efdb6}]{ECA: specifying the rule}
\begin{columns}
...
\begin{frame}[label={sec:orgcaa13ad}]{ECA: rule example}
\begin{columns}
\begin{column}{0.2\columnwidth}
\begin{center}
\begin{tabular}{rrrr}
...
\begin{frame}[label={sec:orgba39e71}]{There is more}
\begin{itemize}
...
\begin{frame}[label={sec:orgb726845},fragile]{Universe}
...
\begin{frame}[label={sec:org2902a40}]{Reversed left?}
\begin{itemize}
...
\begin{frame}[label={sec:orgbb975ff},fragile]{\texttt{fromList}}
 \begin{minted}[]{haskell}
fromList :: a -> [a] -> U a
...
\begin{frame}[label={sec:orge1f4516},fragile]{\texttt{extract}}
 \begin{minted}[]{haskell}
extract :: U a -> a
extract (U _ a _) = a
\end{minted}
...
\begin{frame}[label={sec:org35d9ea8},fragile]{\texttt{Functor}}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orga83b5f4},fragile]{Moving left}
 \begin{minted}[]{haskell}
left :: U a -> U a
...
\begin{frame}[label={sec:orgc4f3642},fragile]{Moving right}
 \begin{minted}[]{haskell}
right :: U a -> U a
...
\begin{frame}[label={sec:orgddf289a},fragile]{Non-totality}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgec3d8ca},fragile]{Defining ECA}
 \begin{minted}[]{haskell}
data Cell
  = Dead
  | Alive
...
\begin{frame}[label={sec:orgc993a33},fragile]{Defining rule}
...
\begin{frame}[label={sec:org1ba0727},fragile]{Rule 110}
 \begin{minted}[]{haskell}
rule110 :: U Cell -> Cell
...
\begin{frame}[label={sec:orgb6879e4},fragile]{Possible solution}
 \begin{enumerate}
...
\begin{frame}[label={sec:orgdd34ae2}]{M-x doctor}
\begin{verse}
...
\begin{frame}[label={sec:org2501ac6},fragile]{All possible shifts}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
duplicate :: U a -> U (U a)
\end{minted}
...
\begin{frame}[label={sec:orgd9c81ba}]{All possible shifts}
\begin{gather*}
...
\begin{frame}[label={sec:org6f08b86}]{Where's the focus?}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:orgc2dcfe5}]{Where's the focus?}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:orgc410aa7}]{Where's the focus}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:org5f8ca27},fragile,t]{\texttt{extend}}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
duplicate :: U a
          -> U (U a)
...
\begin{frame}[label={sec:org5a35b33},fragile]{Iteration}
 \begin{minted}[,escapeinside=||,mathescape=true]{haskell}
...
\begin{frame}[label={sec:orgfde56b8},fragile]{(not so) Final words}
 \begin{itemize}
...
\label{sec:orgc88c1e5}

\begin{frame}[label={sec:org961a650}]{Definition}
...
\begin{frame}[label={sec:org7c4a5b3}]{Rules}
...
\begin{frame}[label={sec:org03b0f1f}]{Example of simulation (1)}
\begin{footnotesize}
  \begin{equation*}
    \begingroup
    \setlength\arraycolsep{0.25pt}
...
\begin{frame}[label={sec:orge32bc08}]{Example of simulation (2)}
\begin{equation*}
  \begingroup
  \setlength\arraycolsep{0.25pt}
  \renewcommand{\arraystretch}{0.5}
...
\begin{frame}[label={sec:org6f3e8c2}]{Example of simulation (3)}
\begin{tiny}
  \begin{equation*}
    \begingroup
    \setlength\arraycolsep{0.25pt}
...
\begin{frame}[label={sec:orgafff089}]{\emph{untitled copy (1)(2) - final}}
\begin{onlyenv}<1>
iTunes anyone?
\end{onlyenv}
...
\begin{frame}[label={sec:org7d4ad23},fragile]{Rolling out U2}
 \begin{minted}[]{haskell}
type U2 a = U (U a)
\end{minted}
\end{frame}

\begin{frame}[label={sec:orgb57392b},fragile]{Moving around in the \sout{complex} Universe}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org590ec0f},fragile]{Game of Life rules}
 \begin{minted}[]{haskell}
rule :: U (U Cell) -> Cell
rule u | ns == 3   = Alive
...
\begin{frame}[label={sec:org94646de},fragile]{Was easy, right?}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgfc06471},fragile]{Extending 2D Universe (1)}
 \begin{minted}[]{haskell}
> :t extend
...
\begin{frame}[label={sec:orga725770},fragile]{Extending 2D Universe (2)}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgad68df3},fragile]{Actually extending 2D Universe}
 \begin{minted}[]{haskell}
newtype U2 a
  = U2
  { getUniverse :: U (U a)
...
\begin{frame}[label={sec:org73ef65c},fragile]{Works}
 \begin{minted}[,escapeinside=||,mathescape=true]{haskell}
...
\begin{frame}[label={sec:orgbe8b1e4}]{(not so) Final words}
\begin{itemize}
...
\label{sec:org3057a4e}

\begin{frame}[label={sec:orga9979cc}]{Definition}
\pause

Do you really need a definition of Bomberman?
\end{frame}

\begin{frame}[label={sec:org63e8c57}]{Definition}
\begin{center}
\includegraphics[width=.9\linewidth]{images/bomberman-1.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgcf9f0b6}]{But we have a different one}
\begin{center}
\includegraphics[height=8cm]{images/bomberman-2.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgad0c268}]{Bells and whistles}
\begin{center}
\includegraphics[height=8cm]{images/bomberman-3.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org86e1d98},fragile]{Reusing U2}
 \begin{minted}[]{haskell}
data Cell
  = Bomberman
  | BombBomberman
...
\begin{frame}[label={sec:orgd2587d8},fragile]{What is bot?}
 \begin{minted}[]{haskell}
bot :: ( MonadIO m
       , MonadReader r m
       , HasLogFunc r
...
\begin{frame}[label={sec:org386aa30}]{What we care about}
\begin{itemize}
...
\begin{frame}[label={sec:org186a484},fragile]{Fatum of the Universe}
 \begin{minted}[]{haskell}
data MarkedCell
  = MarkedCell
  { getCell        :: !Cell
...
\begin{frame}[label={sec:orgd6c66a8},fragile]{Fatum of the Universe}
 \begin{minted}[]{haskell}
> :t extend
...
\begin{frame}[label={sec:orgd84676a},fragile,t]{The bot}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
bot :: ( MonadIO m
       , MonadReader r m
...
\begin{frame}[label={sec:org55f23ce}]{Danger! Live demo}
\emph{Live demos are subject to fail. Accept and forgive. Move on.}
\end{frame}

\begin{frame}[label={sec:org33c69ed}]{(not so) Final words}
\begin{itemize}
...
\section{Finale}
\label{sec:org14d75f0}

\begin{frame}[label={sec:orga2ebe97},fragile]{Commonalities (1)}
 \begin{minted}[]{haskell}
extract :: U a  -> a
extract :: U2 a -> a
\end{minted}
...
\begin{frame}[label={sec:org07e36ab},fragile]{Commonalities (2)}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgeb688a3},fragile]{Comonad}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org360fb82},fragile]{Co-monad}
 \begin{columns}
\begin{column}{0.45\columnwidth}
\begin{minted}[]{haskell}
extract :: f a -> a
...
\begin{frame}[label={sec:org377e22d},fragile]{Universe is Comonad}
...
\begin{frame}[label={sec:org46c2c17},fragile]{Store}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org042c427}]{Many uses}
\begin{itemize}
\item <1-> Instances
\begin{itemize}
...
\begin{frame}[label={sec:org0e56f6d}]{Final words}
\begin{itemize}
...
\begin{frame}[label={sec:orgf5dbac2}]{Grab the code}
\url{https://gitlab.com/d12frosted/talks}
\end{frame}

\section*{References}
\label{sec:orgdad087e}
\begin{frame}[label={sec:org3e132d9}]{References}
\begin{itemize}
...
\section{Questions?}
\label{sec:org923c390}
\section{Thank you}
\label{sec:org36fe564}
\end{document}
