...
\section*{Intro}
\label{sec:orge28c7f3}
\begin{frame}[label={sec:orgb2a7775}]{About me}
\begin{columns}
...
\begin{frame}[label={sec:org8ebfa8a}]{Agenda}
\begin{itemize}
...
\label{sec:orge097b7c}

\begin{frame}[label={sec:org1654c16}]{Cellular Automaton}
...
\begin{frame}[label={sec:org6c5f074}]{Many grids}
\begin{enumerate}
...
\begin{frame}[label={sec:orgb085e3f},t]{Elementary Cellular Automata}
\begin{onlyenv}<1->
...
\begin{frame}[label={sec:orgac4a913},t]{Evolution example}
\begin{onlyenv}<1-3>
...
\begin{frame}[label={sec:orga76190b}]{ECA: specifying the rule}
\begin{columns}
...
\begin{frame}[label={sec:orgcd37b84}]{ECA: rule example}
\begin{columns}
\begin{column}{0.2\columnwidth}
\begin{center}
\begin{tabular}{rrrr}
...
\begin{frame}[label={sec:orgde5fdb8}]{There is more}
\begin{itemize}
...
\begin{frame}[label={sec:orge1edff3},fragile]{Universe}
...
\begin{frame}[label={sec:org042f8af}]{Reversed left?}
\begin{itemize}
...
\begin{frame}[label={sec:org39475c3},fragile]{\texttt{fromList}}
 \begin{minted}[]{haskell}
fromList :: a -> [a] -> U a
...
\begin{frame}[label={sec:orgb2b5a6c},fragile]{\texttt{extract}}
 \begin{minted}[]{haskell}
extract :: U a -> a
extract (U _ a _) = a
\end{minted}
...
\begin{frame}[label={sec:org8003443},fragile]{\texttt{Functor}}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org01db0b8}]{Movement}
\pause
...
\begin{frame}[label={sec:org229fa5c},fragile]{Moving left}
 \begin{minted}[]{haskell}
left :: U a -> U a
...
\begin{frame}[label={sec:org3883272},fragile]{Moving right}
 \begin{minted}[]{haskell}
right :: U a -> U a
...
\begin{frame}[label={sec:org92ef2f1},fragile]{Non-totality}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org8c19332},fragile]{Defining ECA}
 \begin{minted}[]{haskell}
data Cell
  = Dead
  | Alive
...
\begin{frame}[label={sec:org961ba29},fragile]{Defining rule}
...
\begin{frame}[label={sec:orgaa54cd5},fragile]{Rule 110}
 \begin{minted}[]{haskell}
rule110 :: U Cell -> Cell
...
\begin{frame}[label={sec:org98a93d8},fragile]{Possible solution}
 \begin{enumerate}
...
\begin{frame}[label={sec:org37d8438}]{M-x doctor}
\begin{verse}
...
\begin{frame}[label={sec:orga46785c},fragile]{All possible shifts}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
duplicate :: U a -> U (U a)
\end{minted}
...
\begin{frame}[label={sec:orgf828307}]{All possible shifts}
\begin{gather*}
...
\begin{frame}[label={sec:org5da2ac1}]{Where's the focus?}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:orga933da1}]{Where's the focus?}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:orgbba3277}]{Where's the focus}
\begin{gather*}
  \dots\ 0\ 1\ (2)\ 3\ 4\ 5\ \dots \\
...
\begin{frame}[label={sec:orgdd7e49a},fragile,t]{\texttt{extend}}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
duplicate :: U a
          -> U (U a)
...
\begin{frame}[label={sec:org033bea2},fragile]{Iteration}
 \begin{minted}[,escapeinside=||,mathescape=true]{haskell}
...
\begin{frame}[label={sec:org0455878}]{Zipper}
...
\begin{frame}[label={sec:orgea78655},fragile]{(not so) Final words}
 \begin{itemize}
...
\label{sec:orgad1437f}

\begin{frame}[label={sec:org1fa8913}]{Definition}
...
\begin{frame}[label={sec:orgaf35e42}]{Rules}
...
\begin{frame}[label={sec:org4859337}]{Example of simulation (1)}
\begin{footnotesize}
  \begin{equation*}
    \begingroup
    \setlength\arraycolsep{0.25pt}
...
\begin{frame}[label={sec:orgc290455}]{Example of simulation (2)}
\begin{equation*}
  \begingroup
  \setlength\arraycolsep{0.25pt}
  \renewcommand{\arraystretch}{0.5}
...
\begin{frame}[label={sec:org3b15adb}]{Example of simulation (3)}
\begin{tiny}
  \begin{equation*}
    \begingroup
    \setlength\arraycolsep{0.25pt}
...
\begin{frame}[label={sec:orgb441f18}]{\emph{untitled copy (1)(2) - final}}
\begin{onlyenv}<1>
iTunes anyone?
\end{onlyenv}
...
\begin{frame}[label={sec:orgadf40ab},fragile]{Rolling out U2}
 \begin{minted}[]{haskell}
type U2 a = U (U a)
\end{minted}
\end{frame}

\begin{frame}[label={sec:org5067fe1},fragile]{Moving around in the \sout{complex} Universe}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org48bf378},fragile]{Game of Life rules}
 \begin{minted}[]{haskell}
rule :: U (U Cell) -> Cell
rule u | ns == 3   = Alive
...
\begin{frame}[label={sec:org5728861},fragile]{Was easy, right?}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:orgd1e563e},fragile]{Extending 2D Universe (1)}
 \begin{minted}[]{haskell}
> :t extend
...
\begin{frame}[label={sec:org53f0dbc},fragile]{Extending 2D Universe (2)}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org8eca023},fragile]{Actually extending 2D Universe}
 \begin{minted}[]{haskell}
newtype U2 a
  = U2
  { getUniverse :: U (U a)
...
\begin{frame}[label={sec:org9011c86},fragile]{Works}
 \begin{minted}[,escapeinside=||,mathescape=true]{haskell}
...
\begin{frame}[label={sec:orgc58fa8f}]{(not so) Final words}
\begin{itemize}
...
\label{sec:orga7b812c}

\begin{frame}[label={sec:orga4c33c0}]{Definition}
\pause

Do you really need a definition of Bomberman?
\end{frame}

\begin{frame}[label={sec:org6e45283}]{Definition}
\begin{center}
\includegraphics[width=.9\linewidth]{images/bomberman-1.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org35ff32f}]{But we have a different one}
\begin{center}
\includegraphics[height=8cm]{images/bomberman-2.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org97d3951}]{Bells and whistles}
\begin{center}
\includegraphics[height=8cm]{images/bomberman-3.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgb47dee0},fragile]{Reusing U2}
 \begin{minted}[]{haskell}
data Cell
  = Bomberman
  | BombBomberman
...
\begin{frame}[label={sec:org66a2817},fragile]{What is bot?}
 \begin{minted}[]{haskell}
bot :: ( MonadIO m
       , MonadReader r m
       , HasLogFunc r
...
\begin{frame}[label={sec:org4c2d403}]{What we care about}
\begin{itemize}
...
\begin{frame}[label={sec:org51edd6d},fragile]{Fatum of the Universe}
 \begin{minted}[]{haskell}
data MarkedCell
  = MarkedCell
  { getCell        :: !Cell
...
\begin{frame}[label={sec:org768e91e},fragile]{Fatum of the Universe}
 \begin{minted}[]{haskell}
> :t extend
...
\begin{frame}[label={sec:orgc94c584},fragile,t]{The bot}
 \begin{onlyenv}<1>
\begin{minted}[]{haskell}
bot :: ( MonadIO m
       , MonadReader r m
...
\begin{frame}[label={sec:orgdfcb86f}]{Danger! Live demo}
\emph{Live demos are subject to fail. Accept and forgive. Move on.}
\end{frame}

\begin{frame}[label={sec:org96dfeea}]{(not so) Final words}
\begin{itemize}
...
\section{Finale}
\label{sec:org1807233}

\begin{frame}[label={sec:org830435c},fragile]{Commonalities (1)}
 \begin{minted}[]{haskell}
extract :: U a  -> a
extract :: U2 a -> a
\end{minted}
...
\begin{frame}[label={sec:org566b39a},fragile]{Commonalities (2)}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org3325532},fragile]{Comonad}
 \begin{minted}[]{haskell}
...
\begin{frame}[label={sec:org22c1298},fragile]{Co-monad}
 \begin{columns}
\begin{column}{0.45\columnwidth}
\begin{minted}[]{haskell}
extract :: f a -> a
...
\begin{frame}[label={sec:org5b7881b},fragile]{Universe is Comonad}
...
\section*{References}
\label{sec:org8408ad8}
\begin{frame}[label={sec:org641d387}]{References}
\begin{itemize}
...
\section{Questions?}
\label{sec:org64f9239}
\section{Thank you}
\label{sec:org9812133}
\end{document}
